...
figure;
hold on;
histogram(weekday_midpoints, binEdges, 'FaceColor', 'b', 'FaceAlpha', 0.5);
histogram(weekend_midpoints, binEdges, 'FaceColor', 'r', 'FaceAlpha', 0.5);
...
plot(binCenters, rolling_avg_weekday, '-b', 'LineWidth', 2);
plot(binCenters, rolling_avg_weekend, '-r', 'LineWidth', 2);

xlabel('Sleep Midpoint (Time of Day)');
ylabel('Frequency');
title('Overlaid Histogram of Sleep Midpoints with Rolling Average');
legend({'Weekday Data', 'Weekend Data', 'Weekday Rolling Avg', 'Weekend Rolling Avg'});
grid on;
hold off;
