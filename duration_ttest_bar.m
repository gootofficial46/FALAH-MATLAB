...
filename = 'Duration_tTest.xlsx'; % New filename
data = readtable(filename);

% Assign correct column names
weekdays_column = data.Properties.VariableNames{2}; % Mean Weekday Duration (decimal)
weekends_column = data.Properties.VariableNames{4}; % Mean Weekend Duration (decimal)
weekday_sem_column = data.Properties.VariableNames{3}; % Weekday SEM (decimal)
weekend_sem_column = data.Properties.VariableNames{5}; % Weekend SEM (decimal)

% Extract necessary data (still in decimal format)
weekdays_decimal = data.(weekdays_column); % Mean weekday duration (decimal hours)
weekends_decimal = data.(weekends_column); % Mean weekend duration (decimal hours)
weekdays_sem_decimal = data.(weekday_sem_column); % Weekday SEM (decimal hours)
weekends_sem_decimal = data.(weekend_sem_column); % Weekend SEM (decimal hours)
...
% Set y-ticks with reduced intervals
y_min = 0; % Start from 00:00
y_max = ceil(max([weekdays_decimal; weekends_decimal]) * 24);  % Convert to hours
y_tick_interval = 1; % 1-hour intervals
y_ticks = y_min:y_tick_interval:y_max; % Reduced number of ticks
...
bar_data = [weekdays_decimal(whole_population_idx), weekends_decimal(whole_population_idx)];
sem_data = [weekdays_sem_decimal(whole_population_idx), weekends_sem_decimal(whole_population_idx)];
...
bar_data = [weekdays_decimal(internal_idx), weekends_decimal(internal_idx)];
sem_data = [weekdays_sem_decimal(internal_idx), weekends_sem_decimal(internal_idx)];
...
bar_data = [weekdays_decimal(rural_urban_idx), weekends_decimal(rural_urban_idx)];
sem_data = [weekdays_sem_decimal(rural_urban_idx), weekends_sem_decimal(rural_urban_idx)];
...
% Add legend
legend({'Weekday', 'Weekend'}, 'Location', 'northeastoutside', 'FontSize', 10);
...
sgtitle('Mean Sleep Duration by Population', 'FontSize', 26);